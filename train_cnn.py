...
class ConvNet(object):
    ...
    def model_1(self, X, hidden_size):
        ...
        fcl = tf.layers.dense(inputs=X, units=hidden_size, activation=tf.sigmoid)

        fcl = tf.layers.flatten(fcl)
    ...
    def model_2(self, X, hidden_size):
        ...
        fcl = tf.layers.dense(inputs=pool2, units=hidden_size, activation=tf.nn.sigmoid)
    ...
    def model_3(self, X, hidden_size):
        ...
        fcl = tf.layers.dense(inputs=pool2, units=hidden_size, activation=tf.nn.relu)
    ...
    def model_4(self, X, hidden_size, decay):
        ...
        #
        # ----------------- YOUR CODE HERE ----------------------
        input_layer = tf.layers.dense(inputs=X, units=hidden_size, activation=tf.nn.relu)
    ...
    def train_and_evaluate(self, FLAGS, train_set, test_set):
        ...
        with tf.Graph().as_default():
            ...
            correct_prediction = tf.equal(tf.argmax(Y, 1), tf.argmax(trainY, 1))
